      ...
      for (let i = 1,ii =1; i < 11; i++,ii+=10) {
        if ( document.getElementById(`Group${ii}`) ) {
          ...
          if ( (isCountGroupCards !== 0) && (isCountGroupKillCard === 0) ) {
            ...
            var html =`
            <img class="cardDeck-visible mw-100 mh-100 pb-3" src="https://xenotera.herokuapp.com/xenoCards/xenoNoCardDeck.png">
            `
            $("#cardDeck").append(html);
          } if ( (isCountGroupCards === 0) && (isCountGroupKillCard !== 0) ) {
            $("#cardDeck").find(".cardDeck-visible").remove();
            var html =`
            <img class="cardDeck-visible mw-100 mh-100 pb-3" src="https://xenotera.herokuapp.com/xenoCards/xenoRebirthCardDeck.png">
            `
            $("#cardDeck").append(html);
          } if ( (isCountGroupCards !== 0) && (isCountGroupKillCard !== 0) ) {
            $("#cardDeck").find(".cardDeck-visible").remove();
            var html =`
            <img class="cardDeck-visible mw-100 mh-100 pb-3" src="https://xenotera.herokuapp.com/xenoCards/xenoCardDeck.png">
            `
            $("#cardDeck").append(html);
          }
        }
  ...
  $.ajax({
      ...
      success: data => {
        ...
        for (let i = 1,ii =1; i < 11; i++,ii+=10) {
          if ( document.getElementById(`Group${ii}`) ) {
            var html =`
              <img class="usedCardLatest-visible mw-100 mh-100 pb-3" src="https://xenotera.herokuapp.com/xenoCards/xenoCard_${data.isCountGroupUsedCard[i-1]}.png">
            `
