      ...
      for (let i = 1,ii =1; i < 11; i++,ii+=10) {
        if ( document.getElementById(`Group${ii}`) ) {
          ...
          if ( (isCountGroupCards !== 0) && (isCountGroupKillCard === 0) ) {
            ...
            var html =`
            <img class="cardDeck-visible w-42 mb-3 mt-3" src="http://xenotera.herokuapp.com/xenoCards/xenoNoCardDeck.png">
            `
            $("#cardDeck").append(html);
          } if ( (isCountGroupCards === 0) && (isCountGroupKillCard !== 0) ) {
            $("#cardDeck").find(".cardDeck-visible").remove();
            var html =`
            <img class="cardDeck-visible w-42 mb-3 mt-3" src="http://xenotera.herokuapp.com/xenoCards/xenoRebirthCardDeck.png">
            `
            $("#cardDeck").append(html);
          } if ( (isCountGroupCards !== 0) && (isCountGroupKillCard !== 0) ) {
            $("#cardDeck").find(".cardDeck-visible").remove();
            var html =`
            <img class="cardDeck-visible w-42 mb-3 mt-3" src="http://xenotera.herokuapp.com/xenoCards/xenoCardDeck.png">
            `
            $("#cardDeck").append(html);
          }
        }
